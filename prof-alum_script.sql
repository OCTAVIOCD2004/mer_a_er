...
CREATE TABLE IF NOT EXISTS `Profesor-Grupo_alumnos`.`P-G` (
  ...
  CONSTRAINT `fk_Profesor_has_Grupos alumnos_Profesor`
    FOREIGN KEY (`Profesor_Cod_profesor`)
    REFERENCES `Profesor-Grupo_alumnos`.`Profesor` (`Cod_profesor`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Profesor_has_Grupos alumnos_Grupos alumnos1`
    FOREIGN KEY (`Grupos alumnos_Cod-grupo`)
    REFERENCES `Profesor-Grupo_alumnos`.`Grupos alumnos` (`Cod-grupo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
